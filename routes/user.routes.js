...
userRouter.post("/login",async(req,res)=>{
    ...
    if(isUserPresent){
       ...
       if(result){
        const token = jwt.sign({email,userId:isUserPresent._id,role:isUserPresent.role},process.env.key,{expiresIn:60*2})
        const refreshtoken = jwt.sign({email,userId:isUserPresent._id},process.env.refreshkey,{expiresIn:60*4})
